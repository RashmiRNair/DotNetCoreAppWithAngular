...
namespace DatingWebApplication.API.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        public async Task<IEnumerable<Message>> GetMessageThread(int userId, int recipientId)
        {
           ...
           .ThenInclude(u => u.Photos)
           .Include(u => u.Recipient)
           .ThenInclude(u => u.Photos).Where(m => m.RecipientId == userId && m.RecipientDeleted == false && m.SenderId == recipientId ||
        ...
        {
            var user = await _context.Users.Include(p => p.Photos).FirstOrDefaultAsync(e => e.Id == id);
            return user;
        }
